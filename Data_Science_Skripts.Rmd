...
p <- ggplot(data = berlin_map_bezirksregion) +
  geom_sf(aes(
...
### Tabellenansicht und Balkendiagram der Absoluten Diebstähle im Bezug zu den Geodaten
...
split_data_bezirksregion <- split(berlin_map_bezirksregion, cut(1:nrow(berlin_map_bezirksregion), breaks = 5, labels = FALSE))
...
# Erstelle die Diagramme mit einem durchgehenden Farbverlauf
plots_bezirksregion_anzahl <- lapply(1:5, function(i) {
  ggplot(split_data_bezirksregion[[i]], aes(x = BZR_NAME, y = anzahl_bezirksregion, fill = anzahl_bezirksregion)) +
    geom_bar(stat = "identity") +
    scale_fill_gradient(low = "cyan4", high = "chartreuse3", limits = c(min_val, max_val)) +  # Farbverlauf über alle Plots hinweg
    labs(
...
# Ergebnisse anzeigen
print(plots_bezirksregion_anzahl[[1]])
print(plots_bezirksregion_anzahl[[2]])
print(plots_bezirksregion_anzahl[[3]])
print(plots_bezirksregion_anzahl[[4]])
print(plots_bezirksregion_anzahl[[5]])
```
...
ggplot(berlin_map_bezirk, aes(x = BEZ, y = anzahl_bezirk, fill = anzahl_bezirk)) +
  ...
  scale_fill_gradient(low = "cyan4", high = "chartreuse3") +  # Farbverlauf
  labs(
...
```
### Tabelle und Balkendiagramm des Durchschnitts, Bezirksregionen
...
# Aufgeteilter Plot Bezirksregion
split_data_bezirksregion_durchschnitt <- split(berlin_map_durschnitt_bezirksregion, cut(1:nrow(berlin_map_durschnitt_bezirksregion), breaks = 5, labels = FALSE))
...
# Erstelle die Diagramme mit einem durchgehenden Farbverlauf
plots_bezirksregion_durchschnitt <- lapply(1:5, function(i) {
  ggplot(split_data_bezirksregion_durchschnitt[[i]], aes(x = BZR_NAME, y = durchschnitt_promille, fill = durchschnitt_promille)) +
    geom_bar(stat = "identity") +
    scale_fill_gradient(low = "cyan4", high = "chartreuse3", limits = c(min_val, max_val)) +  # Farbverlauf über alle Plots hinweg
    labs(
...
# Ergebnisse anzeigen
print(plots_bezirksregion_durchschnitt[[1]])
print(plots_bezirksregion_durchschnitt[[2]])
print(plots_bezirksregion_durchschnitt[[3]])
print(plots_bezirksregion_durchschnitt[[4]])
print(plots_bezirksregion_durchschnitt[[5]])
```
